<script lang="ts">
  ...
  import { NeoCard, NeoList, NeoListSearch } from '@dvcol/neo-svelte';
  ...
  // TODO : tags in item - icon, engagement
  // TODO : mark as read
  ...
  const lines = $derived.by(() => {
    ...
    if (innerWidth.current < 1000) return 5;
    if (innerWidth.current < 1200) return 6;
    return 10;
  });
...
<style lang="scss">
  .side-feed-list {
    ...
    :global(.neo-list-item-media) {
      ...
      will-change: scale, opacity, margin, padding, max-width;
